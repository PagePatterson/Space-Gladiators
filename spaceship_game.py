...
class PewPew(pygame.sprite.Sprite):
    ...
    def moveRandom(self):
        ...
        if self.posx < 1:
            self.randMove = random.choice([1,5,6]) # the choices are straight or diagonally away from the edge of the screen
        if self.posx > 799:
            self.randMove = random.choice([0,4,7])
        if self.posy < 1:
            self.randMove = random.choice([3,4,7])
        if self.posy > 399:
            self.randMove = random.choice([2,5,6])
...
class Enemy(PewPew):
    ...
    def __init__(self):
        ...
        self.pos = (self.posx, self.posy)
        self.rect = self.surf.get_rect(center = self.pos)
        self.randMove = random.randint(0,3)
        self.health = 2 
...
class GameMenu():
    ...
    def __init__(self):
        ...
        self.score = body_font.render(f'highscore:{manager.highscore}', True, (64, 64, 64))
...
class GameActive():
    ...
    def __init__(self):
        ...
        self.pause1 = body_font.render(f'press space to exit game', True, 'Purple')
        ...
        self.space = body_font.render(f'press space to continue', True, 'Purple')
        self.space_rect = self.space.get_rect(center = (400, 250))
